...
<body>
                                ...
                                <div class="card">
                                    ...
                                    <tr>
                                        ...
                                        <th>Status</th>
                                               ...
                                               <tbody>
                                    <?php 
                                    $sel_list = mysqli_query($con,"select * from tbl_order");
                                    $i=0;
								   ...
								   <td>
								  <button class="preview_d btn btn-primary" data-id="<?php echo $row['id'];?>" data-toggle="modal" data-target="#myModal">Preview</button></td>
								  
								  <td>
								  <a href="?did=<?php echo $row['id'];?>" class="btn btn-danger">Delete</a></td>
								  <td>
								  <?php 
								  if($row['status'] == 'pending')
								  {
								  ?>
								  <a href="?s=proceesing&pid=<?php echo $row['id'];?>" class="btn btn-info"> Processing </a>
								   <a href="?s=completed&pid=<?php echo $row['id'];?>" class="btn btn-success"> Completed </a>
								  <?php } else if($row['status']  == 'processing') {?>
								  <a href="?s=pending&pid=<?php echo $row['id'];?>" class="btn btn-primary"> Pending </a>
								   <a href="?s=completed&pid=<?php echo $row['id'];?>" class="btn btn-success"> Completed </a>
								  <?php } else { ?>
								   <a href="?s=pending&pid=<?php echo $row['id'];?>" class="btn btn-primary"> Pending </a>
								   <a href="?s=processing&pid=<?php echo $row['id'];?>" class="btn btn-success"> Processing </a>
								  <?php } ?>
								  </td>
    ...
    {
        $qid = $_GET['did'];
       mysqli_query($con,"delete from tbl_order where id=".$qid."");
        
        ?>
        <script>
            alert('successfully Delete Order');
          // alert('You Use Demo So It Can not Delete Selected Order!!');
            window.location.href="olist.php";
        </script>
        <?php 
    }
    ?> 
  ...
  <div class="modal-dialog modal-lg">
    ...
    {
        $qid = $_GET['s'];
         mysqli_query($con,"update tbl_order set status='".$qid."' where id=".$_GET['pid']."");
        
        ?>
        <script>
           alert('successfully Change Status ');
           //alert('You Use Demo So It Can not Change Status Of Order!!');
            window.location.href="olist.php";
        </script>
        <?php 
    }
...
{
$("#dt-opt").on("click", ".preview_d", function()
	{
